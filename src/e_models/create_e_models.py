...
WHERE EXISTS (
    ...
    WHERE
        ...
        {table_names.e_models}.{columns.prdn.name} = firmid_count.{columns.prdn.name} AND
        {table_names.e_models}.{columns.assg_seq.name} = firmid_count.{columns.assg_seq.name}
);
    ''')


def create_eK_models_table(fh):
    """
    """
    fh.write(
        f'''
...
-- The actual models
CREATE TABLE {table_names.e1_models} AS
SELECT
